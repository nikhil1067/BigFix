...
def setup_service_logger():
    ...
    log_file = os.path.join(script_dir, r"C:\BigFix-SX Adapter\service.log")
    ...
    if len(sys.argv) > 1:
        ...
        try:
            if args.providecredentials:
                logger.info("Credentials management initiated.")
                main(provide_credentials=True)
                logger.info("Credentials provided successfully. Initiating authentication process.")
            elif args.run:
