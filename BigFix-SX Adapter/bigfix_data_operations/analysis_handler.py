...
class BigFixAnalysisHandler:
    def __init__(self, base_url, username, password, site_name, hash_value):
...
<BES xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="BES.xsd">
    ...
    def post_analysis(self, payload):
        ...
        try:
            response = self.APIRequestHandler.request(method="POST", endpoint=endpoint, username=self.username, password=self.password, headers=headers, data=payload, verify=False)
    ...
    def put_analysis(self, analysis_id, payload):
        ...
        try:
            response = self.APIRequestHandler.request(method="PUT", endpoint=endpoint, username=self.username, password=self.password, headers=headers, verify=False)
